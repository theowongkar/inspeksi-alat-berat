<form action="{{ route('dashboard.inspection.store') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
    ...
    <input type="hidden" name="equipment_type_id" value="{{ $equipmentTypes->firstWhere('name', 'Excavator')->id }}">
    <input type="hidden" name="equipment_type" value="Excavator">
    ...
    {{-- Equipment Selection --}}
    <div class="p-5 bg-white rounded-xl shadow-md">
        <label class="block text-sm font-medium text-gray-700">Select Excavator Unit</label>
        <select name="equipment_id"
            class="mt-1 block w-full border border-gray-300 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500 px-2 py-1">
            <option value="">-- Select Equipment --</option>
            @foreach ($equipments->where('equipment_type_id', $equipmentTypes->firstWhere('name', 'Excavator')->id) as $eq)
    ...
    <div class="p-5 bg-white rounded-xl shadow-md">
        ...
        <div class="grid grid-cols-2 gap-4">
            @php
                $excavatorTypeId = $equipmentTypes->firstWhere('name', 'Excavator')->id;
                $filteredItems = $lists->where('equipment_type_id', $excavatorTypeId);
            @endphp
